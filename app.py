...
def render_dashboard():
    ...
    col1.metric("相談者数", int(counts.get("clients_count", 0)))
    col2.metric("案件数", int(counts.get("cases_count", 0)))
    col3.metric("進行中案件", int(counts.get("open_cases_count", 0)))
    col4.metric("要確認案件", int(counts.get("need_check_count", 0)))
    ...
    if need_check.empty:
        ...
        st.dataframe(need_check, use_container_width=True, hide_index=True)
    ...
    with left_mid:
        ...
        if hearing.empty:
            ...
            st.dataframe(hearing, use_container_width=True, hide_index=True)
    ...
    with right_mid:
        ...
        if do_not.empty:
            ...
            st.dataframe(do_not, use_container_width=True, hide_index=True)
